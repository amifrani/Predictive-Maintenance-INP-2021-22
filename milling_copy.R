...
## THIS SCRIPT IS A "SANITIZED" VERSION OF THE REAL SCRIPT IN THAT IT FOCUSES ON THE IMPLEMENTATION OF THE METHODOLOGY ONLY.
...
# For the three kinds of signals (acoustic emission, vibration and motor current), (Traini et al., 2019) and (Cai et al., 2020) rely on the following statistical properties: 
  ...
  ## Sum
  ## Minimum
  ## Spectral skewness - not done
  ## Spectral kurtosis - not done
  ## Wavelet energy - not done
  ## The features computed by tsfeatures() (TSFeatures package) by default
...
## What variables have the greatest influence (Pearson's coefficient > 0.25 or < -0.25) on VB?
cor(data_with_temporal_features[-3], data_with_temporal_features$VB)
capture.output(cor(data_with_temporal_features[-3], data_with_temporal_features$VB), file = "Correlation-with-VB.csv")
cor(data_with_temporal_features[-3], data_with_temporal_features$VB)[which((cor(data_with_temporal_features[-3], data_with_temporal_features$VB)[,1] > 0.25) | (cor(data_with_temporal_features[-3], data_with_temporal_features$VB))[,1] < -0.25), 0]
## Now, what correlations exist between those variables?
correlated_with_VB <- cbind(data_with_temporal_features$run, data_with_temporal_features$time, data_with_temporal_features$material, data_with_temporal_features$max_AE_spindle, data_with_temporal_features$skewness_vib_spindle, data_with_temporal_features$kurtosis_vib_spindle, data_with_temporal_features$e_acf10_smcAC, data_with_temporal_features$entropy_smcAC, data_with_temporal_features$x_acf10_smcAC, data_with_temporal_features$diff1_acf1_smcAC, data_with_temporal_features$diff1_acf10_smcAC, data_with_temporal_features$diff2_acf1_smcAC, data_with_temporal_features$diff2_acf10_smcDC, data_with_temporal_features$e_acf10_smcDC, data_with_temporal_features$diff1_acf10_smcDC, data_with_temporal_features$diff2_acf10_smcDC, data_with_temporal_features$trend_vib_spindle, data_with_temporal_features$spike_vib_spindle, data_with_temporal_features$curvatuve_vib_spindle, data_with_temporal_features$diff2_acf1_vib_spindle, data_with_temporal_features$trend_AE_table, data_with_temporal_features$e_acf1_AE_table, data_with_temporal_features$entropy_AE_table, data_with_temporal_features$x_acf10_AE_table, data_with_temporal_features$diff1_acf1_AE_table, data_with_temporal_features$diff1_acf10_AE_table, data_with_temporal_features$diff2_acf10_AE_table, data_with_temporal_features$diff2_acf1_AE_table, data_with_temporal_features$diff2_acf10_AE_table, data_with_temporal_features$trend_AE_table, data_with_temporal_features$e_acf1_AE_spindle, data_with_temporal_features$entropy_AE_spindle, data_with_temporal_features$x_acf10_AE_spindle, data_with_temporal_features$diff1_acf1_AE_spindle, data_with_temporal_features$diff1_acf10_AE_spindle, data_with_temporal_features$diff2_acf10_AE_spindle)
colnames(correlated_with_VB) <- c("run",                   
                                  "time                  " ,
                                  "material" ,
                                  "max_AE_spindle        " ,
                                  "skewness_vib_spindle  " ,
                                  "kurtosis_vib_spindle  " ,
                                  "e_acf10_smcAC         " ,
                                  "entropy_smcAC         " ,
                                  "x_acf10_smcAC         " ,
                                  "diff1_acf1_smcAC      " ,
                                  ...
                                  "trend_vib_spindle     " ,
                                  "spike_vib_spindle     " ,
                                  "curvature_vib_spindle " ,
                                  "diff2_acf1_vib_spindle" ,
                                  "trend_AE_table        " ,
                                  "e_acf1_AE_table       " ,
                                  "entropy_AE_table      " ,
                                  "x_acf10_AE_table      " ,
                                  "diff1_acf1_AE_table   " ,
                                  "diff1_acf10_AE_table  " ,
                                  "diff2_acf1_AE_table   ", 
                                  "diff2_acf10_AE_table",   
                                  "trend_AE_spindle"   ,    
                                  "e_acf1_AE_spindle" ,     
                                  "entropy_AE_spindle",    
                                  "x_acf10_AE_spindle",    
                                  "diff1_acf1_AE_spindle",
                                  "diff1_acf10_AE_spindle",
                                  "diff2_acf10_AE_spindle")
ggcorrplot(cor(correlated_with_VB), hc.order = TRUE)
...
adjusted_rsquared_LR_1_test <- 1 - (1 - rsquared_LR_1_test)*(length(LR_1_results$case) - 1)/(length(LR_1_results$case) - 4 - 1)
...
rsquared_LR_2_test <- 1 - sum((LR_2_results$actual - LR_2_results$predicted)^2) / (sum((LR_2_results$predicted - mean(LR_2_results$actual))^2) + sum((LR_2_results$actual - LR_2_results$predicted)^2))
adjusted_rsquared_LR_2_test <- 1 - (1 - rsquared_LR_2_test)*(length(LR_2_results$case) - 1)/(length(LR_2_results$case) - 5 - 1)
...
rsquared_LR_7_test <- 1 - sum((LR_7_results$actual - LR_7_results$predicted)^2) / (sum((LR_7_results$predicted - mean(LR_7_results$actual))^2) + sum((LR_7_results$actual - LR_7_results$predicted)^2))
adjusted_rsquared_LR_7_test <- 1 - (1 - rsquared_LR_7_test)*(length(LR_7_results$case) - 1)/(length(LR_7_results$case) - 6 - 1)
...
rsquare_LR_1_exp_11 <- rsquare(a = LR_1_results[LR_1_results$case == 11, "actual"], b = LR_1_results[LR_1_results$case == 11, "predicted"])
rsquare_LR_1_exp_12 <- rsquare(a = LR_1_results[LR_1_results$case == 12, "actual"], b = LR_1_results[LR_1_results$case == 12, "predicted"])
rsquare_LR_1_exp_15 <- rsquare(a = LR_1_results[LR_1_results$case == 15, "actual"], b = LR_1_results[LR_1_results$case == 15, "predicted"])
rsquare_LR_1_exp_16 <- rsquare(a = LR_1_results[LR_1_results$case == 16, "actual"], b = LR_1_results[LR_1_results$case == 16, "predicted"])
rsquare_LR_1_test <- rsquare(a = LR_1_results$actual, b = LR_1_results$predicted)
...
rsquare_LR_1_train <- rsquare(a = LR_1_results$actual, b = LR_1_results$predicted)
...
## Case 11
results_with_time_and_case <- data.frame(time = test$time, case = test$case, actual = test.r, prediction_NN1 = pr.nn_, prediction_NN2 = pr.nn_2, prediction_NN3 = pr.nn_3)
plot_NN_exp_11 <- ggplot(data=results_with_time_and_case[results_with_time_and_case$case == 11,], mapping=aes(x=time, y=actual), xlab = "Temps (mn)", ylab = "VB (mm)") + geom_point(color = "blue") + geom_line(color = "red", data = results_with_time_and_case[results_with_time_and_case$case == 11, ], aes(x = time, y = prediction_NN1)) + geom_line(color = "orange", data = results_with_time_and_case[results_with_time_and_case$case == 11, ], aes(x = time, y = prediction_NN2)) + geom_line(color = "cyan", data = results_with_time_and_case[results_with_time_and_case$case == 11, ], aes(x = time, y = prediction_NN3)) + theme_set(theme_gray()) + xlab("Temps (mn)") + ylab("VB (mm)") + ggtitle("Exp. 11")
...
MAE_NN_3_exp_11 <- mae(actual = results_with_time_and_case[results_with_time_and_case$case == 11, ]$actual, predicted = results_with_time_and_case[results_with_time_and_case$case == 11, ]$prediction_NN3)

## Case 12
plot_NN_exp_12 <- ggplot(data=results_with_time_and_case[results_with_time_and_case$case == 12,], mapping=aes(x=time, y=actual)) + geom_point(color = "blue") + geom_line(color = "red", data = results_with_time_and_case[results_with_time_and_case$case == 12, ], aes(x = time, y = prediction_NN1)) + geom_line(color = "orange", data = results_with_time_and_case[results_with_time_and_case$case == 12, ], aes(x = time, y = prediction_NN2)) + geom_line(color = "cyan", data = results_with_time_and_case[results_with_time_and_case$case == 12, ], aes(x = time, y = prediction_NN3)) + theme_set(theme_gray()) + xlab("Temps (mn)") + ylab("VB (mm)") + ggtitle("Exp. 12")
...
MAE_NN_3_exp_12 <- mae(actual = results_with_time_and_case[results_with_time_and_case$case == 12, ]$actual, predicted = results_with_time_and_case[results_with_time_and_case$case == 12, ]$prediction_NN3)

## Case 15
plot_NN_exp_15 <- ggplot(data=results_with_time_and_case[results_with_time_and_case$case == 15,], mapping=aes(x=time, y=actual)) + geom_point(color = "blue") + geom_line(color = "red", data = results_with_time_and_case[results_with_time_and_case$case == 15, ], aes(x = time, y = prediction_NN1)) + geom_line(color = "orange", data = results_with_time_and_case[results_with_time_and_case$case == 15, ], aes(x = time, y = prediction_NN2)) + geom_line(color = "cyan", data = results_with_time_and_case[results_with_time_and_case$case == 15, ], aes(x = time, y = prediction_NN3)) + theme_set(theme_gray()) + xlab("Temps (mn)") + ylab("VB (mm)") + ggtitle("Exp. 15")
...
MAE_NN_3_exp_15 <- mae(actual = results_with_time_and_case[results_with_time_and_case$case == 15, ]$actual, predicted = results_with_time_and_case[results_with_time_and_case$case == 15, ]$prediction_NN3)

## Case 16
plot_NN_exp_16 <- ggplot(data=results_with_time_and_case[results_with_time_and_case$case == 16, ], mapping=aes(x=time, y=actual)) + geom_point(color = "blue") + geom_line(color = "red", data = results_with_time_and_case[results_with_time_and_case$case == 16, ], aes(x = time, y = prediction_NN1)) + geom_line(color = "orange", data = results_with_time_and_case[results_with_time_and_case$case == 16, ], aes(x = time, y = prediction_NN2)) + geom_line(color = "cyan", data = results_with_time_and_case[results_with_time_and_case$case == 16, ], aes(x = time, y = prediction_NN3)) + theme_set(theme_gray()) + xlab("Temps (mn)") + ylab("VB (mm)") + ggtitle("Exp. 16")
ggarrange(plot_NN_exp_11, plot_NN_exp_12, plot_NN_exp_15, plot_NN_exp_16)

## Let's compute each NN's RMSE with respect to case 16
...
MAE_NN_3_exp_16 <- mae(actual = results_with_time_and_case[results_with_time_and_case$case == 16, ]$actual, predicted = results_with_time_and_case[results_with_time_and_case$case == 16, ]$prediction_NN3)


MAE_NN_1_test <- mae(actual = test.r, predicted = pr.nn_)
MAE_NN_2_test <- mae(actual = test.r, predicted = pr.nn_2)
MAE_NN_3_test <- mae(actual = test.r, predicted = pr.nn_3)



avg_accuracy_NN_1_test <-MAE_NN_1_test / mean(test.r)
avg_accuracy_NN_2_test <-MAE_NN_2_test / mean(test.r)
avg_accuracy_NN_3_test <- MAE_NN_3_test / mean(test.r)
...
##################### LSTM-NN HYBRID MODEL-BASED VB REGRESSION ###########################
## We will train an LSTM-based encoder to encode the sensor signals (motor currents, vibrations and acoustic emission).
## Once trained, the outputs of the encoder will serve as an input to a non-linear regression NN.
...
# Comparing our RMSEs, MAEs and average accuracies to those obtained by (Cai et al., 2020)
RMSE_this <- data.frame(Algorithms = c("LR_1", "NN_1", "NN_2", "NN_3", "SVM_1", "LR_ref", "CNN_ref", "SVM_ref", "MLP_ref"), RMSE = signif(c(RMSE_LR_1_test,RMSE_NN_1_test, RMSE_NN_2_test, RMSE_NN_3_test, RMSE_SVM_1_test, 0.2146, 0.2143, 0.1945, 0.2168), c(4, 4, 4, 4, 4, 4, 4, 4, 4)))
p <- ggplot(data=RMSE_this, aes(x=Algorithms, y=RMSE, fill=Algorithms)) +
  geom_bar(stat="identity", col="black")+
  geom_text(aes(label=RMSE),vjust=-0.3, size=3.5)+
  theme_minimal()

MAE_this <- data.frame(Algorithms = c("LR_1", "NN_1", "NN_2", "NN_3", "SVM_1", "LR_ref", "CNN_ref", "SVM_ref", "MLP_ref"), MAE = signif(c(MAE_LR_1_test,MAE_NN_1_test, MAE_NN_2_test, MAE_NN_3_test, MAE_SVM_1_test, 0.1879, 0.1835, 0.1700, 0.1830), c(4, 4, 4, 4, 4, 4, 4, 4, 4)))
p <- ggplot(data=MAE_this, aes(x=Algorithms, y=MAE, fill=Algorithms)) +
  geom_bar(stat="identity", col="black")+
